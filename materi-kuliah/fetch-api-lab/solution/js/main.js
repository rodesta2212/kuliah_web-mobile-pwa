...
var app = (function() {
  ...
  function fetchJSON() {
    fetch('examples/animals.json') // 1 URL
    .then(validateResponse) // 2 response
    .then(readResponseAsJSON) // 3 mengambil data -> response.json()
    .then(logResult) // 4 tampilkan di log -> console.log
    .catch(logError);
  }
  ...
  function fetchImage() {
    fetch('examples/kitten.jpg') // url
    .then(validateResponse) // response
    .then(readResponseAsBlob) // response.blob() = binary large object
    .then(showImage)
    .catch(logError);
  }
  ...
  function fetchText() {
    fetch('examples/words.txt') //url untuk mengambil words.txt
